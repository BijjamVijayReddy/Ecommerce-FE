...
const ProductFilter = () => {
  ...
  return (
    <div className="">
      ...
      <div className="w-full flex justify-center mt-[150px] mb-2">
        <input
          type="text"
          placeholder="Search products..."
          value={searchTerm}
          onChange={handleSearch}
          className="border border-[brown] p-2 rounded-md w-1/2 focus:outline-none"
        />
      </div>
      ...
      <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 shadow-md lg:grid-cols-4 p-4'>
        {filteredProducts.map((item) => (
          <div key={item.id} className="relative overflow-hidden border p-5 align-center shadow-md rounded-xl bg-opacity-50 ">
            <img
              src={item.imageUrl}
              alt="product"
            ...
            <div className="absolute top-[-150px] left-[-10px] inset-1 flex items-center justify-start p-5">
              <h5 className=" font-medium text-white bg-black bg-opacity-50 px-2 py-1 rounded-lg capitalize">{item.category}</h5>
            </div>
            <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center items-center opacity-0 rounded-lg hover:opacity-100 transition-opacity duration-300">
              ...
              <button className="mt-2 px-4 py-2 bg-[brown] text-white rounded" onClick={() => cartHandler(item)} >Add To Cart</button>
            </div>
          </div>
        ))}
      </div>
